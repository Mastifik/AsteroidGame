    ...
    class Program
    {
        ...
        private static double ComputerLongDataValue(int Count, ILogger Log)
        {
            ...
            for (var i = 0; i < Count; i++)
            {
                ...
                if (i > 500)
                    throw new InvalidOperationException("Число итерации оказалось слишком большим",
                        new ArgumentException("Число итерации было указано больше 500 и равно {Count}", nameof(Count)));
            }
   ...
   public class ComputeExceptionException : ApplicationException
    {
        ...
        protected ComputeExceptionException(SerializationInfo info, StreamingContext context) : base(info, context)
        {
